class Train
  ...
  def station_info(index)
    puts "Сейчас поезд на станции #{station.name}."
    pev_station(index)
    next_station(index)
  end

  def pev_station(index)
    puts "Предыдущая станция - #{route.stations[index - 1].name}." if index != 0
  end
